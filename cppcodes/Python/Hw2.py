...
class Catalog:
    ...
    def addCourse(self, cid, cname, credits):
        if cid not in self.courseOfferings:
            new_course = Course(cid, cname, credits)
            self.courseOfferings[cid] = new_course
    ...
    def _loadCatalog(self, file):
        target_path = os.path.join(os.path.dirname(__name__), file)
        ...
        for course in course_info:
            ...
            new_course_info.append(new_course)
         # only way I know to remove the new line character from each string and keeping it in a list


        
        for course in new_course_info:
            course_credentials = course.split(',')
            self.addCourse(course_credentials[0], course_credentials[1], int(course_credentials[2]))
    ...
    def __str__(self):
        if self.courses:
            ...
            for course in self.courses:
                course_holder.append(course)
            courses = '; '.join(course_holder)
            return courses
        return 'No courses'
...
class Loan:
    ...
    def __getloanID(self):
        loanID = random.randrange(10000, 100000)
        return loanID
...
class Staff(Person):
    ...
    def id(self):
        ...
        first_and_last_name = self.name.split()
        ...
        return '905' + first_initial + last_initial + f'{self.ssn[-4:]}'
    ...
    def createStudent(self, person):
        if isinstance(person, Person):
            new_student = Student(person.name, person.ssn, 'Freshmen')
            return new_student
...
class Student(Person):
    ...
    def id(self):
        ...
        return first_initial + last_initial + f'{self.ssn[-4:]}'
    ...
    def registerSemester(self):
        if self.active == True and self.hold == False:
            ...
            if self.semesters == {}:
                self.semesters[1] = new_semester
            else:
                semesters = self.semesters.keys()
                latest_semester = max(semesters)
                self.semesters[latest_semester + 1] = new_semester
            
            semesters = self.semesters.keys()
            latest_semester = max(semesters)
            if latest_semester == (1 or 2):
                self.classCode = 'Freshman'
            elif latest_semester == (3 or 4):
                self.classCode = 'Sophomore'
            elif latest_semester == (5 or 6):
                self.classCode = 'Junior'
            elif latest_semester >= 7:
    ...
    def enrollCourse(self, cid, catalog):
        ...
        semesters = self.semesters.keys()
        latest_semester = max(semesters)
        course_to_add = catalog.courseOfferings[cid]

        if cid not in self.semesters[latest_semester].courses:
            self.semesters[latest_semester].addCourse(course_to_add)
            course_cost = int(course_to_add.credits) * self.account.CREDIT_PRICE
            self.account.chargeAccount(course_cost)
            return 'Course added successfully'
        else:
    ...
    def dropCourse(self, cid):
        ...
        if cid not in self.semesters[latest_semester].courses:
            ...
            course_to_drop = self.semesters[latest_semester].courses[cid]
            self.semesters[latest_semester].dropCourse(course_to_drop)

            refund = (course_to_drop.credits * self.account.CREDIT_PRICE) / 2
    ...
    def getLoan(self, amount):
        ...
        if self.semesters[latest_semester].isFullTime == False:
